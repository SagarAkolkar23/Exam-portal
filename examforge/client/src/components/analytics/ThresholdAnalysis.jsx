...
export default function ThresholdAnalysis({ totalMarks = 100 }) {
  ...
  const {
    ...
    error: reportError,
  } = useGetThresholdReport(threshold, isQueryEnabled);
  ...
  const displayedStudents = useMemo(() => {
    ...
    const list =
      activeSlice === "above"
        ? reportData.aboveThreshold
        : reportData.belowThreshold;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-100 pb-3 gap-3">
        <div>
          ...
          <p className="text-xs text-slate-400 font-medium">
            Benchmark and split student cohorts based on a customizable
            threshold mark.
          </p>
        </div>
          ...
          <div className="relative flex items-center">
            <input
              ...
              onChange={(e) => {
                const val = Math.max(
                  0,
                  Math.min(totalMarks, Number(e.target.value)),
                );
                setThreshold(val);
              }}
      ...
      {!isQueryEnabled ? (
        <div className="flex flex-col items-center justify-center py-8 gap-1.5 text-center border border-dashed border-slate-200 rounded-2xl bg-slate-50/20">
          ...
          <h4 className="text-xs font-bold text-slate-500">
            Awaiting Selector Input
          </h4>
          <p className="text-[10px] text-slate-400 max-w-[280px]">
            Please select an exam from the dashboard header filters to load the
            threshold analyzer.
          </p>
        </div>
      ) : loadingReport ? (
        <div className="flex flex-col items-center justify-center py-10 gap-2">
          <div className="flex flex-col items-center justify-center py-24 gap-4 bg-white border border-slate-200/60 rounded-3xl shadow-sm">
            <div className="w-20 h-20 rounded-full bg-indigo-50 flex items-center justify-center text-4xl shadow-sm"></div>
            <h2 className="text-xl font-bold text-slate-700">
              No exam selected
            </h2>
            <p className="text-slate-400 text-sm font-medium max-w-sm text-center">
              Please select an exam from the filters above to view real-time
              analytics.
            </p>
          </div>
        </div>
        ...
        <div className="text-xs bg-red-50 text-red-700 border border-red-100 p-3 rounded-xl">
          Error retrieving cohort performance:{" "}
        ...
        <div className="flex flex-col items-center justify-center py-8 text-center border border-dashed border-slate-200 rounded-2xl">
          <span className="text-xl">👥</span>
          <h4 className="text-xs font-bold text-slate-500">
            No Students Match Active Filters
          </h4>
          <p className="text-[10px] text-slate-400 max-w-[280px]">
            There are no student accounts registered matching your active filter
            configuration.
          </p>
        </div>
      ) : (
            ...
            <div className="w-full grid grid-cols-2 gap-2 mt-2">
              ...
              >
                ...
                <span className="text-xs font-black text-slate-800 mt-0.5">
                  {reportData.aboveCount} students
                </span>
              </button>
              <button
              ...
              >
                ...
                <span className="text-xs font-black text-slate-800 mt-0.5">
                  {reportData.belowCount} students
                </span>
              </button>
            </div>
          </div>
          ...
          <div className="md:col-span-7 bg-white border border-slate-100 rounded-2xl p-4 flex flex-col min-h-[220px]">
            ...
            <div className="flex items-center justify-between pb-2.5 border-b border-slate-100">
              <span className="text-xs font-black text-slate-700 uppercase tracking-wider flex items-center gap-1.5">
                <span
                  className={`w-2.5 h-2.5 rounded-full ${activeSlice === "above" ? "bg-[#10b981]" : "bg-[#f43f5e]"}`}
                />
                {activeSlice === "above" ? "Scored ≥" : "Scored <"} {threshold}{" "}
                Marks
              </span>
              ...
              <div className="relative w-36">
                ...
                />
                <svg
                  className="w-2.5 h-2.5 text-slate-400 absolute left-2 top-1/2 -translate-y-1/2"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
            ...
            <div className="flex-1 mt-2.5 max-h-[145px] overflow-y-auto pr-0.5 space-y-1.5">
              {displayedStudents.length === 0 ? (
                ...
                displayedStudents.map((s) => (
                  <div
                    key={s._id}
                    className="flex items-center justify-between bg-slate-50/50 border border-slate-100/60 hover:border-slate-200 rounded-xl px-3 py-1.5 transition-all"
                  >
                    <div className="min-w-0">
                      <div className="flex items-center gap-1.5">
                        <span
                          className={`text-[8px] font-black border px-1.5 py-0.5 rounded uppercase tracking-wider shrink-0 ${
                            activeSlice === "above"
                    ...
                    <div className="text-right shrink-0">
                      <span
                        className={`text-[11px] font-black block ${activeSlice === "above" ? "text-emerald-600" : "text-rose-500"}`}
                      >
                        {s.attempted
                          ? `${s.score} / ${totalMarks}`
                          : `0 / ${totalMarks}`}
